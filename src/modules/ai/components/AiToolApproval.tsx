...
const TOOL_META: Record<string, { label: string; icon: typeof FilePlusIcon }> =
  {
    ...
    execute_plan: { label: "Approve action plan", icon: ToolsIcon },
    create_mindmap: { label: "Create mind map", icon: ToolsIcon },
  };
    ...
    return (
      <div className="space-y-1.5">
        <div className="font-medium text-[11px] text-foreground">
          {String(input.title ?? "Action plan")}
        </div>
        <ol className="space-y-1.5 text-[11px] text-muted-foreground">
          {steps.map((step, index) => (
            <li key={`${String(step.kind)}-${index}`} className="flex gap-2">
              <span className="size-4 shrink-0 rounded-full bg-muted text-center text-[10px] leading-4 text-foreground">
                {index + 1}
              </span>
              <span className="min-w-0 break-words">
                {step.kind === "workspace"
                  ? `Create ${String(step.terminal_count)} terminal(s) at ${String(step.folder)}`
                  : step.kind === "mindmap"
                    ? `Draw mind map: ${String(step.title)}`
                  : step.kind === "browser"
                    ? `Open ${String(step.url)}`
                    : "Open Architecture"}
              </span>
            </li>
          ))}
