...
export function AiInputBar({
  ...
  return (
    <div className="shrink-0 border-t border-border/60 bg-card/70 px-3 py-2.5">
      <div
        className={cn(
          "flex flex-col gap-2 rounded-xl border border-border/70 bg-background px-2.5 py-2 shadow-sm",
          "transition-colors focus-within:border-primary/35 focus-within:ring-2 focus-within:ring-primary/10",
        )}
      >
        <ChipsRow
          files={c.files}
              ...
              <textarea
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Enter" && !e.shiftKey) {
                    e.preventDefault();
                    if (isBusy) return;
                    c.submit({ openMini: openMiniOnSubmit });
                  }
                }}
                placeholder="Ask cmdSpace anything   -   # for snippets and commands, @ for files"
                rows={1}
                className={cn(
                  "max-h-40 min-h-7 flex-1 resize-none bg-transparent py-1 text-[13px] leading-relaxed outline-none break-all",
                  "placeholder:text-muted-foreground/60",
                )}
              />
              {showAgentSwitcher ? <AgentSwitcher /> : null}
              <Button
                type="button"
                size="icon"
                variant={isBusy ? "destructive" : "default"}
                disabled={isBusy ? !onStop : !c.canSend}
                onClick={() => {
                  if (isBusy) onStop?.();
                  else c.submit({ openMini: openMiniOnSubmit });
                }}
                className={cn(
                  "size-7 shrink-0 rounded-lg",
                  isBusy && "text-destructive hover:bg-destructive/10 hover:text-destructive",
                )}
                aria-label={isBusy ? "Stop response" : "Send message"}
                title={isBusy ? "Stop response" : "Send message"}
              >
                <HugeiconsIcon
                  icon={isBusy ? StopCircleIcon : ArrowUpIcon}
                  size={14}
                  strokeWidth={2}
                />
              </Button>
