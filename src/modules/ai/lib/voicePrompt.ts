...
const VOICE_PROMPT_SYSTEM = `You are cmdSpace Voice Agent. Convert a user's spoken request into the next compact task brief for the active coding CLI agent.
...
export function buildVoicePromptContext(
  ...
  return [
    ...
    recentDrafts.length
      ? `Recent Voice Agent task drafts, newest first:\n${recentDrafts
