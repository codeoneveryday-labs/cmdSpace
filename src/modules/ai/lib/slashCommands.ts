...
const INIT_PROMPT = `Scan this workspace and produce CMDSPACE.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap CMDSPACE.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const CMDSPACE_CMD_RE =
  /^<cmdspace-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<cmdspace-command name="${name}" />\n\n${prompt}`;
}
