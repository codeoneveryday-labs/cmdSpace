...
type ComposerCtx = {
  ...
  isBusy: boolean;
  submit: (options?: { openMini?: boolean }) => void;
  stop: () => void;
  voice: Voice;
  canSend: boolean;
};
...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("cmdspace:ai-attach-file", onAttach);
    return () => window.removeEventListener("cmdspace:ai-attach-file", onAttach);
  ...
  const submit = (options: { openMini?: boolean } = {}) => {
    if (isBusy) return;
    ...
    // the prompt to the CMDSPACE.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<cmdspace-command name="${outcome.commandName}" />`;
        }
      }
    }
    ...
    if (options.openMini !== false && !store.mini.open) store.openMini();
    setValue("");
    setFiles([]);
    setPickedSnippets([]);
    setPickedCommands([]);
