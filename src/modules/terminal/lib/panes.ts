...
export function findLeafLastCommand(n: PaneNode, id: PaneId): string | undefined {
  if (isLeaf(n)) return n.id === id ? n.lastCommand : undefined;
  for (const c of n.children) {
    const found = findLeafLastCommand(c, id);
    if (found !== undefined) return found;
  }
  return undefined;
}
...
export function setLeafLastCommand(
  n: PaneNode,
  id: PaneId,
  lastCommand: string,
): PaneNode {
  if (isLeaf(n)) {
    if (n.id !== id || n.lastCommand === lastCommand) return n;
    return { ...n, lastCommand };
  }
  let changed = false;
  const next = n.children.map((c) => {
    const u = setLeafLastCommand(c, id, lastCommand);
    if (u !== c) changed = true;
    return u;
  });
  return changed ? { ...n, children: next } : n;
...
export function setSplitChildSizes(
  tree: PaneNode,
  splitId: PaneId,
  childSizes: Record<PaneId, number>,
): PaneNode {
  ...
  if (tree.id === splitId) {
    ...
    const children = tree.children.map((child) => {
      const size = childSizes[child.id];
      if (size === undefined || child.size === size) return child;
      changed = true;
      return { ...child, size };
    });
  ...
  let changed = false;
  const children = tree.children.map((child) => {
    const next = setSplitChildSizes(child, splitId, childSizes);
