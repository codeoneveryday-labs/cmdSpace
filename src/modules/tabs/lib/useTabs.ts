...
export type ArchitectureTerminalDockTabs = {
  id: string;
  kind: "tabs";
  terminalIds: string[];
...
export type ArchitectureTerminalDockSplit = {
  id: string;
  kind: "split";
...
export type ArchitectureTerminalDockNode =
  | ArchitectureTerminalDockTabs
  | ArchitectureTerminalDockSplit;

export type ArchitectureTerminalDockGroup = {
  id: string;
  x: number;
  y: number;
  width: number;
...
function countSavedPaneLayoutLeaves(value: unknown): number {
  ...
  return node.children.reduce(
    (sum, child) => sum + countSavedPaneLayoutLeaves(child),
    0,
  );
}
...
function createPaneTree(
  ...
  if (countSavedPaneLayoutLeaves(savedLayout) === paneCount) {
    ...
    const buildFromSavedLayout = (value: unknown): PaneNode | null => {
      ...
      if (savedNode.kind === "leaf") {
        const savedPane = panes?.find((p) => p.paneIndex === currentPaneIdx);
        currentPaneIdx++;
        return {
          kind: "leaf",
          id: nextId(),
      ...
      if (
        savedNode.kind !== "split" ||
        (savedNode.dir !== "row" && savedNode.dir !== "col") ||
        !Array.isArray(savedNode.children)
      ) {
        return null;
      }
      const children = savedNode.children
        .map(buildFromSavedLayout)
        .filter((child): child is PaneNode => child !== null);
      if (children.length === 0) return null;
      if (children.length === 1) {
        const onlyChild = children[0];
        return size !== undefined ? { ...onlyChild, size } : onlyChild;
      }
      return {
        kind: "split",
        id: nextId(),
  ...
  const buildStack = (children: PaneNode[], dir: SplitDir): PaneNode => {
  ...
  const columnCounts = Array.from(
    { length: columns },
    (_, column) => baseRows + (column < extraRows ? 1 : 0),
  ).filter((columnCount) => columnCount > 0);

  let currentPaneIdx = 0;
  const buildColumn = (leafCount: number): PaneNode =>
    buildStack(
...
export function useTabs(initial?: Partial<TerminalTab>) {
  ...
  const newArchitectureTab = useCallback((diagram?: ArchitectureDiagram, title = "Architecture") => {
    const id = nextIdRef.current++;
    setTabs((t) => [
      ...t,
      {
  ...
  const reorderTab = useCallback(
    (
      ...
      setTabs((curr) => {
        const from = curr.findIndex((tab) => tab.id === draggedId);
        const targetIndex = curr.findIndex((tab) => tab.id === targetId);
        if (from < 0 || targetIndex < 0 || from === targetIndex) return curr;
        const next = [...curr];
  ...
  const setLeafLastCommand = useCallback((leafId: number, lastCommand: string) => {
    setTabs((curr) => {
      let changed = false;
      const next = curr.map((t) => {
        if (t.kind !== "terminal" || !hasLeaf(t.paneTree, leafId)) return t;
        const paneTree = setLeafLastCommandInTree(t.paneTree, leafId, lastCommand);
  ...
  const setTerminalPaneTree = useCallback((tabId: number, paneTree: PaneNode) => {
    setTabs((curr) =>
      curr.map((t) => {
        ...
        const activeLeafId = ids.includes(t.activeLeafId)
          ? t.activeLeafId
          : ids[0];
        return {
          ...t,
          ...
          activeLeafId,
          maximizedLeafId:
            t.maximizedLeafId !== undefined && ids.includes(t.maximizedLeafId)
              ? t.maximizedLeafId
              : undefined,
        };
      }),
      ...
      return curr.map((x) =>
        x.id === tab.id
          ? {
              ...
              maximizedLeafId: (x as TerminalTab).maximizedLeafId === leafId ? undefined : (x as TerminalTab).maximizedLeafId,
            }
          : x,
      );
    });
      ...
      return curr.map((x) =>
        x.id === tabId
          ? {
              ...
              paneTree: newTree,
              activeLeafId: newActive,
              maximizedLeafId: (x as TerminalTab).maximizedLeafId === target ? undefined : (x as TerminalTab).maximizedLeafId,
            }
          : x,
      );
    });
  ...
  const toggleMaximizePane = useCallback((leafId: number): void => {
    setTabs((curr) =>
      curr.map((t) => {
        if (t.kind !== "terminal" || !hasLeaf(t.paneTree, leafId)) return t;
        return {
          ...t,
          maximizedLeafId: t.maximizedLeafId === leafId ? undefined : leafId,
        };
      }),
    );
  }, []);
