...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S cmdspace-bin";
    case "debian":
      return `sudo apt install ./cmdspace_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./cmdspace-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `cmdSpace v${manual.version} is available`
                  : `cmdSpace v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart cmdSpace to finish installing."
              : downloading
                ? progress !== null
