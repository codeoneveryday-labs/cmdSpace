# cmdspace-shell-integration (bashrc)
#
...
if [ -z "$__CMDSPACE_HOOKS_LOADED" ]; then
  ...
  _cmdspace_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_cmdspace_urlencode "$PWD")"
    if [ -z "$__CMDSPACE_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __CMDSPACE_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_cmdspace_precmd:"*) ;;
    *) PROMPT_COMMAND="_cmdspace_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
