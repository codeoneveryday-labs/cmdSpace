...
**Context:** `crates/cmdspace-remote-protocol` is the shared v2 protocol contract. The desktop remote web client already proves desired lifecycle behavior: authenticate after hello, attach one selected session, deduplicate output, and reattach from sequence zero after a host runtime restart.

**Approach:** Add one dependency-free Rust crate, `cmdspace-remote-client`. It returns explicit actions for a later platform/WebSocket adapter rather than performing network or platform work internally.
...
- `cargo test --manifest-path crates/cmdspace-remote-client/Cargo.toml`
- `cargo test --manifest-path crates/cmdspace-remote-protocol/Cargo.toml`
...
The native-client core is implemented in `crates/cmdspace-remote-client`. Its
