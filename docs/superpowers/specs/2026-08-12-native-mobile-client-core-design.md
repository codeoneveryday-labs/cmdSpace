...
future native UI will render. It owns remote connection state and session
...
This phase does not add an Android Gradle project, iOS Xcode project, a
WebSocket dependency, device authentication storage, or terminal rendering.
Those are platform adapters; mixing any of them into the state layer would make
protocol lifecycle tests require a mobile target.
