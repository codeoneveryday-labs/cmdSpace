...
This project uses the current Harness core binary at `scripts/bin/harness`.
Run it through `pnpm harness:status`, `pnpm harness:update`, and
`pnpm harness:doctor`. The binary is present in this repository and is the
default upstream interface.

## Optional Harness CLI compatibility layer

The legacy `scripts/bin/harness-cli` described below is an optional SQLite
control plane, not part of the installed Harness core. Do not invoke it unless
the compatibility bundle has been deliberately installed after resolving any
upstream Harness update conflicts.

The commands below apply only to projects that installed the optional
compatibility layer. Those projects use the prebuilt binary at
